<?php
namespace Ftob\OAuth2\Client\Tests;

use Ftob\OAuth2\Client\Providers\OAuthProvider;
...
class OAuthProviderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->provider->setDomain('https://example.com');
        $response = m::mock('Psr\Http\Message\ResponseInterface');
        ...
        $this->provider->setHttpClient($client);
        $token = $this->provider->getAccessToken('authorization_code', ['code' => 'mock_authorization_code']);
        $this->assertEquals($this->provider->getDomain().'/login/oauth/authorize', $this->provider->getBaseAuthorizationUrl());
        $this->assertEquals($this->provider->getDomain().'/login/oauth/access_token', $this->provider->getBaseAccessTokenUrl([]));
        $this->assertEquals($this->provider->getDomain().'/api/v1/user', $this->provider->getResourceOwnerDetailsUrl($token));
